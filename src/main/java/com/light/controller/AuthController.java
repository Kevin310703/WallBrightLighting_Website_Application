...
import com.light.request.LoginRequest;
import com.light.request.RegisterRequest;
    ...
    public String login(@ModelAttribute LoginRequest request, HttpServletResponse response, Model model, RedirectAttributes redirectAttributes) {
        try {
            ...
            if (user.getRole().getId() == 1) {
                ...
                return String.format("redirect:/dashboard/index?year=%d&month=%d", currentYear, currentMonth);
            } else {
                return "redirect:/";
            }
        } catch (Exception ex) {
    ...
    public String register(@ModelAttribute RegisterRequest request, Model model) {
        ...
        if (!errorList.isEmpty()) {
            ...
            while (true) {
                ...
                if (userRepository.findAll()
                        .stream()
                        .noneMatch(u -> Objects.equals(u.getVerifyCode(), finalVerifyCode))) {
                    break;
                }
            }
